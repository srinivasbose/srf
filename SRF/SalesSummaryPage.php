						...
						<?php
								...
								{		
									...
									if($_POST["billDate"] != ''){
										...
										$sql1="SELECT * from sales_vw where date = '$date' $cond";
										$result=mysql_query($sql1);
										if(mysql_num_rows($result) != 0){
											?>
											<br>
											<h4>Sales Bills on <?php echo date_format(new DateTime($date),"d-m-Y") ?> </h4>
											<br>
										...
										$sql1="SELECT * from sales_vw where date >= '$fromDate' and date <= '$endDate' $cond";
										$result=mysql_query($sql1);
										if(mysql_num_rows($result) != 0){
											?>
											<br>
											...
											?>
											<p><b>No Sales bills from <?php echo date_format(new DateTime($fromDate),"d-m-Y") ?> to <?php echo date_format(new DateTime($endDate),"d-m-Y") ?>.</b></p>
											<?php
										}
									}else if($_POST["billMonth"] != '' ){
										$first_day = $_POST['billMonth'] . "-01";
										$period = date_format(new DateTime($first_day), "Y-m-d");
										$sql1="SELECT * from sales_vw WHERE date >= '$period' AND date  < '$period' + INTERVAL 1 MONTH $cond";
										$result=mysql_query($sql1);
										if(mysql_num_rows($result) != 0){
											?>
											<br>
											<h4>Sales Bills from <?php echo date_format(new DateTime($_POST['billMonth']),"m-Y") ?> </h4>
											<br>
											<table border='1' style='border-collapse: collapse'>
											<th>Bill No.</th><th>Name</th><th>Item</th><th>Brand</th><th>Category</th><th>Quantity</th>
											...
											?>
											<p><b>No Sales bills from <?php echo date_format(new DateTime($_POST['billMonth']),"m-Y") ?> .</b></p>
											<?php
										}
